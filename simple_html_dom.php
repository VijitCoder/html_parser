<?php
/**
 * Слегка улучшенная версия {@link https://sourceforge.net/projects/simplehtmldom/ Simple HTML DOM Parser}
 *
 * - поправил загрузку файла в file_get_html(), там было $offset = -1, что приводит к загрузке только последнего символа.
 * - включил по умолчанию сохранение переносов строк ($stripRN = false)
 * - отключил по умолчанию приведение тегов и атрибутов к нижнему регистру ($lowercase = false)
 * - отключил по умолчанию закрытие тегов ($forceTagsClosed = false)
 * - добавил метод удаления элемента, removeNode().
 */
 ...
 * This tag_start gets counted AFTER \r\n have been crushed out, and after the remove_noice calls so it will
 * not reflect the REAL position of the tag in the source, it will almost always be smaller by some amount.
 * We use this to determine how far into the file the tag in question is. This "percentage will never be accurate
 * as the $dom->size is the "real" number of bytes the dom was created from. But for most purposes, it's a really good
 * estimation.
 *
 ...
 *
 * NOTE: If the user's system has a routine called `get_last_retrieve_url_contents_content_type` available, we will
 * assume it's returning the content-type header from the last transfer or curl_exec(), and we will parse that and
 * use it in preference to any other method of charset detection.
 *
...
class simple_html_dom_node
{
     ...
     * @param int  $deep
     */
    function dump($showAttr = true, int $deep = 0)
    {
     ...
     * @return string|void
     */
    function dump_node(bool $echo = true)
    {
        $string = $this->tag;
     ...
     * @return null
     */
    function parentNode(?$parent = null)
    {
    ...
    function find_ancestor_tag($tag)
    {
        ...
        while (!is_null($resulturnDom)) {
            if (is_object($debugObject)) {
                $debugObject->debugLog(2, "Current tag is: " . $resulturnDom->tag);
            }

            if ($resulturnDom->tag == $tag) {
                break;
            }
            $resulturnDom = $resulturnDom->parent;
        }
        return $resulturnDom;
    }
    ...
    /**
     * Get node's outer text (with tag)
     */
    function outerText()
    {
        global $debugObject;
        ...
        if ($this->dom && $this->dom->nodes[$this->_[HDOM_INFO_BEGIN]]) {
            $result = $this->dom->nodes[$this->_[HDOM_INFO_BEGIN]]->makeup();
        } else {
            $result = '';
        }
    ...
    function text()
    {
        ...
        if (!is_null($this->nodes)) {
            foreach ($this->nodes as $node) {
                $result .= $this->convert_text($node->text());
            }
            ...
            if ($tag == 'span') {
                $result .= $this->dom->default_span_text;
            }
        }
        return $result;
    }
    ...
    function XMLText()
    {
        return str_ireplace(['<![CDATA[', ']]>'], '', $this->innerText());
    }
    ...
    function makeup()
    {
        ...
        foreach ($this->attr as $key => $val) {
            ...
            $result .= $this->_[HDOM_INFO_SPACE][$i][0];
            ...
            if ($val === true) {
                ...
                $result .= $key . $this->_[HDOM_INFO_SPACE][$i][1] . '=' . $this->_[HDOM_INFO_SPACE][$i][2]
                    . $quote . $val . $quote;
            }
        }
        $result = $this->dom->restore_noise($result);
        return $result . $this->_[HDOM_INFO_ENDSPACE] . '>';
    }
    ...
    function find($selector, $idx = null, $lowercase = false)
    {
        ...
        for ($c = 0; $c < $count; ++$c) {
            ...
            // used to be: if (($level=count($selectors[0]))===0) return [];
            $level = count($selectors[$c]);
            if (!($level && isset($this->_[HDOM_INFO_BEGIN]))) {
                return [];
            }
            ...
            for ($l = 0; $l < $level; ++$l) {
                $result = [];
                foreach ($head as $k => $v) {
                    $node = ($k === -1) ? $this->dom->root : $this->dom->nodes[$k];
                    //PaperG - Pass this optional parameter on to the seek function.
                    $node->seek($selectors[$c][$l], $result, $lowercase);
                }
                $head = $result;
            }
     ...
     * @param bool $lowercase
     */
    protected function seek($selector, &$result, $lowercase = false)
    {
        ...
        $end = empty($this->_[HDOM_INFO_END]) ? 0 : $this->_[HDOM_INFO_END];
        if ($end == 0) {
            $parent = $this->parent;
            ...
            if ($pass && $key && $val && $val !== '*') {
                ...
                if (is_object($debugObject)) {
                    $debugObject->debugLog(
                        2,
                        "testing node: " . $node->tag . " for attribute: " . $key . $exp . $val
                ...
                if (!$check && strcasecmp($key, 'class') === 0) {
                    foreach (explode(' ', $node->attr[$key]) as $k) {
                        // Without this, there were cases where leading, trailing, or double spaces lead to our
                        // comparing blanks - bad form.
                        if (!empty($k)) {
                            $check = $lowercase
        ...
        if (is_object($debugObject)) {
            $debugObject->debugLog(1, "EXIT - ret: ", $result);
        }
    }
    ...
    protected function parse_selector($selector_string)
    {
        ...
        // Paperg: Add the colon to the attrbute, so that it properly finds <tag attr:ibute="something" > like google
        // does.
        // Note: if you try to look at this attribute, you MUST use getAttribute since $dom->x:y will fail
        // the php syntax check.
        // Notice the \[ starting the attbute? and the @? following? This implies that an attribute can begin with
        // an @ sign that is not captured.
        // This implies that an html attribute specifier may start with an @ sign that is NOT captured by
        // the expression. Farther study is required to determine of this should be documented or removed.
        // $pattern = "/([\w-:\*]*)(?:\#([\w-]+)|\.([\w-]+))?(?:\[@?(!?[\w-]+)(?:([!*^$]?=)[\"']?(.*?)[\"']?)?\])?([\/, ]+)/is";
        $pattern = "/([\w-:\*]*)(?:\#([\w-]+)|\.([\w-]+))?(?:\[@?(!?[\w-:]+)(?:([!*^$]?=)[\"']?(.*?)[\"']?)?\])?([\/, ]+)/is";
    ...
    function convert_text($text)
    {
        ...
        if (is_object($debugObject)) {
            $debugObject->debugLog(3, "source charset: {$sourceCharset}, target charset: {$targetCharset}");
        }
    ...
    */
     ...
     * @return array an array containing the 'height' and 'width' of the image on the page or -1 if we can't
     * figure it out.
     */
    function get_display_size()
    {
        ...
        // Look at all the parent tags of this image to see if they specify a class or id that has an img selector that
        // specifies a height or width. Note that in this case, the class or id will have the img subselector for it
        // to apply to the image.

        // Ridiculously far future development
        // If the class or id is specified in a SEPARATE css file thats not on the page, go get it and do what we were
        // just doing for the ones on the page.
...
class simple_html_dom
{
    ...
    protected function copy_until_char($char)
    {
        ...
        if (($pos = strpos($this->doc, $char, $this->pos)) === false) {
            $result = substr($this->doc, $this->pos, $this->size - $this->pos);
            $this->char = null;
            $this->pos = $this->size;
            return $result;
        }
